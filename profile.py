...
# --------------------------------------------------------------------------- #
# CONFIG — point these at a real build, then run `scalene run profile_pipeline.py`
# --------------------------------------------------------------------------- #
SOURCE_DIR = Path(
    r"C:\Users\ohp460\Documents\Code\ampm-analysis\data\JR327_02062026\[3] Export Packets"
)  # folder DataStore reads
STL_PATH = Path(
    r"C:\Users\ohp460\Documents\Code\ampm-analysis\data\JR327_02062026\Layout_1.stl"
)  # full-plate STL for masking
LAYER_LO, LAYER_HI = 101, 300  # inclusive layer range
...
MASK_CACHE = None  # set to a .pkl path to cache the sliced mask
# --------------------------------------------------------------------------- #
# Clustering runs on DEFAULT_COLUMNS = ("Demand X", "Demand Y", "Z"), all of
# which the queried/masked frame already carries. Pass columns=[...] to
# cluster_dbscan_chunked if your frame names them differently.
# --------------------------------------------------------------------------- #
...
def main() -> None:
    ...
    # Direct call (not cluster_or_load) so the work actually runs every time —
    # a warm cache would skip exactly the stage we want to measure.
