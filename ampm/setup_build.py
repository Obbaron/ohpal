...
Walks a build directory to locate the source data, STL, and QuantAM
...
def _find_parts_csv(build_dir: Path) -> Path:
    ...
    quantam_csvs = [p for p in csvs if _is_quantam_csv(p)]
    ...
    raise ValueError(
        ...
        + "\n".join(f"  {c}" for c in quantam_csvs)
...
def _extract_layer_thickness(parts_csv: Path) -> float:
    ...
    """
    with open(parts_csv, "r", encoding="utf-8-sig") as file:
...
def create_config(
    ...
    *,
    ...
    """
    Auto-detect build files and write a config.toml into the build directory.

    Parameters
    ----------
    build_dir : str or Path
        Root directory of the build data.
